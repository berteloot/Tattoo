...
async function deploySampleGalleryToProduction() {
  try {
    console.log('🚀 Deploying sample gallery items to production...\n');
    ...
    console.log(`📊 Found ${artists.length} approved artists in production`);
    
    if (artists.length === 0) {
      console.log('❌ No approved artists found in production');
      return;
    }
    ...
    console.log(`\n📊 Total gallery items in production: ${totalItems}`);
    ...
    console.log(`📊 Approved gallery items in production: ${approvedItems}`);
    
  } catch (error) {
    console.error('❌ Error deploying sample gallery items to production:', error);
  } finally {
    await prisma.$disconnect();
  }
}

deploySampleGalleryToProduction();
