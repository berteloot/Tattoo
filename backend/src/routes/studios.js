...
/**
 * @route   POST /api/studios/contact
 * @desc    Send contact email to studio from client (simplified version)
 * @access  Public (with rate limiting)
 */
router.post('/contact', [
  detectScraping,
  contactInfoLimiter,
  body('studioId').isString().notEmpty().withMessage('Studio ID is required'),
  body('studioName').isString().notEmpty().withMessage('Studio name is required'),
  body('studioEmail').isEmail().withMessage('Valid studio email is required'),
  body('message').isString().notEmpty().withMessage('Message is required'),
  ...
  body('clientPhone').optional().isString().withMessage('Phone must be a string'),
  body('subject').optional().isString().withMessage('Subject must be a string'),
  body('studioAddress').optional().isString().withMessage('Studio address must be a string')
], async (req, res) => {
  try {
    // Check for validation errors
    const errors = validationResult(req);
